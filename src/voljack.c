...
static void set_mute(app_t *app, int mute) {
        atomic_store_explicit(&app->muted, mute ? 1 : 0, memory_order_relaxed);
        if (mute) {
                atomic_store_explicit(&app->target_gain, 0.0f, memory_order_relaxed);
        }
}

static void set_dim(app_t *app, int dim) {
        float db = atomic_load_explicit(&app->dim_db, memory_order_relaxed);
        ...
        atomic_store_explicit(&app->muted, dim ? 1 : 0, memory_order_relaxed);
        if (dim) {
                if (is_neg_inf || isinf(current_db)) {
                        atomic_store_explicit(&app->target_gain, 0.0f, memory_order_relaxed);
                } else {
                        float dimmed_db = current_db + db;
                        if (dimmed_db < app->min_db) dimmed_db = app->min_db;
                        atomic_store_explicit(&app->target_gain, db_to_gain(0, dimmed_db), memory_order_relaxed);
                }
        } else {
                if (is_neg_inf || isinf(current_db)) {
                        atomic_store_explicit(&app->target_gain, 0.0f, memory_order_relaxed);
                } else {
                        atomic_store_explicit(&app->target_gain, db_to_gain(0, current_db), memory_order_relaxed);
                }
        }
}
...
static void handle_command(app_t *app, const char *cmd, int fd) {
        ...
        if (strcmp(tmp, "get") == 0) {
                ...
                int is_neg_inf = atomic_load_explicit(&app->db_is_neg_inf, memory_order_relaxed);
                float current_db = atomic_load_explicit(&app->current_db, memory_order_relaxed);
                int muted = atomic_load_explicit(&app->muted, memory_order_relaxed);
                format_db(dbbuf, sizeof(dbbuf), is_neg_inf, current_db);
                if (muted) snprintf(out, sizeof(out), "%s dB (muted)\n", dbbuf);
                else snprintf(out, sizeof(out), "%s dB\n", dbbuf);
                send_reply(fd, out);
                return;
        }
        ...
        if (strcmp(tmp, "unmute") == 0) {
                set_mute(app, 0);
                send_reply(fd, "OK\n");
                return;
        }

        if (strcmp(tmp, "toggle") == 0) {
                int muted = atomic_load_explicit(&app->muted, memory_order_relaxed);
                set_mute(app, !muted);
                send_reply(fd, "OK\n");
                return;
...
static void usage(const char *argv0) {
        fprintf(stderr,
                        ...
                        "  %s mute [--socket PATH]\n"
                        "  %s unmute [--socket PATH]\n"
                        "  %s toggle [--socket PATH]\n"
                        "  %s dim [--socket PATH]\n"
                        "  %s undim [--socket PATH]\n",
...
int main(int argc, char **argv) {
        ...
        if (argc >= 2 &&
                ...
                if (strcmp(cmd, "get") == 0) {
                        ...
                        return send_client_command(socket_path, "mute");
                } else if (strcmp(cmd, "unmute") == 0) {
                        return send_client_command(socket_path, "unmute");
                } else if (strcmp(cmd, "toggle") == 0) {
                        return send_client_command(socket_path, "toggle");
                } else if (strcmp(cmd, "dim") == 0) {
                        return send_client_command(socket_path, "dim");
                } else if (strcmp(cmd, "undim") == 0) {
                        return send_client_command(socket_path, "undim");
                }
        }
        ...
        atomic_init(&g_app.muted, 0);
        atomic_init(&g_app.dim_db, DEFAULT_DIM_DB);
